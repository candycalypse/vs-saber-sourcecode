...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-100).loadGraphic(Paths.image('saber_bg'));
		bg.scrollFactor.x = 0;
		...
		//if you see this i'm sorry kade
		grpWeekText = new FlxTypedGroup<MenuItem>();
	//	add(grpWeekText);

		var saber:FlxSprite = new FlxSprite(325, 700).loadGraphic(Paths.image('week0'));
		saber.setGraphicSize(Std.int(saber.width * 1.5));
		saber.updateHitbox();
		//saber.screenCenter();
		saber.antialiasing = FlxG.save.data.antialiasing;
		add(saber); 

		FlxTween.tween(saber,{y: 585}, 1, {ease: FlxEase.expoInOut});
